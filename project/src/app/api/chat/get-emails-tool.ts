...
export const getEmailsTool = tool({
  description:
    "Fetch full content of specific emails by their IDs. Use this after searching or filtering to retrieve complete email bodies. Optionally include entire conversation threads.",
  ...
  execute: async ({ ids, includeThread }) => {
    ...
    if (includeThread && results.length > 0) {
      ...
      results = emails
        .filter((email) => threadIds.includes(email.threadId))
